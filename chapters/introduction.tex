...
if exact solution are available, it is possible to quantify the bias that they introduce and
to verify if extrapolations or bias reduction techniques actually converge to correct values.
...
and the pure gauge $U(1)$ theory is, among them, the one that has the most straightforward implementation.
...
His algorithm uses the local gauge invariance to map the system to a one dimensional XY model, and updates it using the extension of the Wolff algorithm.
