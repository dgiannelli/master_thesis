...
The site $s + \hat\mu$ with $\mu \in {1,2}$ corresponds to the point that has the $\mu$th coordinate increased by $a$.
...
if a local gauge transform $G(s) \in U(1)$ applied to site $s$, it affects all link variables connected to it.
...
%in fact, calling $\mathcal C$ the generic path, and $\mathcal L_1, \mathcal L_2, \ldots$ ,
...
\begin{equation}\label{eq:plaq}
    ...
    \end{aligned}
\end{equation}
If $a \to 0$, it is then related the gauge field tensor evaluated at $s$.
